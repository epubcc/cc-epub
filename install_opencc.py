...
为什么会出现 "Unable to locate package opencc"？
  ...
  2) 包名不同               -> opencc / opencc-data / libopencc-dev
...
def test_convert():
    ...
    try:
        ...
        out = OpenCC("tw2sp.json").convert("這是一個繁體測試，包含軟體")
        ok = "软件" in out
...
def main():
    ...
    attempts = []
    if have("pkg"):
        attempts.append(("Termux pkg", "pkg install -y opencc", "pkg update -y"))
    if have("apt"):
        attempts.append(("Debian/Ubuntu apt",
                         "sudo apt install -y opencc opencc-data libopencc-dev",
                         "sudo apt update -y"))
    if have("dnf"):
        attempts.append(("Fedora dnf", "sudo dnf install -y opencc", "sudo dnf makecache"))
    if have("pacman"):
        attempts.append(("Arch pacman", "sudo pacman -S --noconfirm opencc", "sudo pacman -Sy"))
    if sys.platform == "darwin" and have("brew"):
        attempts.append(("macOS brew", "brew install opencc", "brew update"))
