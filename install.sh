...
# --- 安装系统依赖 ---
echo ""
echo "[2/5] 正在更新包列表并安装依赖..."
apt update -y
apt install -y unzip zip opencc imagemagick
...
echo ""
echo "后续步骤："
echo "  1. 执行 termux-setup-storage 并允许存储权限（如果尚未执行）"
echo "  2. 执行 source ~/.bashrc 刷新配置，或重启 Termux"
echo "  3. 执行 cc --doctor 检查环境"
