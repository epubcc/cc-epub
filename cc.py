...
"""
港台繁体 EPUB 转换工具 v2.0
功能：繁体→简体 | 竖排→横排 | 首行缩进2em | 清理全角空格 | 标点规范化 | 字体统一 | 字号统一
...
def convert_text(text):
    ...
    try:
        ...
        if result.returncode == 0:
            return normalize_punctuation(result.stdout)
        return text
...
def clean_css_file(css_content):
    """清洗外部CSS文件：清除竖排、字体、字号等声明，追加覆盖规则"""
...
def clean_html_content(html_content, convert_t2s=True):
    ...
    # 第3步：清除 <p> 标签的 style 和 class（统一由注入 CSS 控制）
    ...
    # 第4步：清除其他标签的 font-family / font-size 内联样式
    ...
    # 第5步：修正 html/body 标签的 writing-mode
...
def process_opf(opf_content):
    ...
    def convert_meta_content(match):
        ...
        if re.search(r'[\u4e00-\u9fff]', content_val):
            converted = convert_text(content_val)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ('--help', '-h', 'help'):
        ...
        print("示例: cc 原子习惯.epub")
        print("      cc ~/storage/shared/Download/书名.epub")
        print("")
